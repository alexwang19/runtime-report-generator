...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        var (secureUrlAuthority, apiToken, outputFileName, reportID) = ParseArgs(args);
        ...
        Stream csvFileStream = await apiService.DownloadReportAsync(secureUrlAuthority, httpClient, reportID);
        List<RuntimeResultInfo> runtimeResults = await apiService.GetRuntimeWorkloadScanResultsList(secureUrlAuthority, httpClient);

        var writer = new StreamWriter(outputFileName);
        int counter = 0;
        if (csvFileStream != null)
        {
            Console.WriteLine("Beginning matching process...");
            ...
            if (duplicates.Any())
            {
                ...
                {
                    Console.WriteLine($"Duplicate found: {duplicate.K8SClusterName}, {duplicate.K8SNamespaceName}, {duplicate.K8SWorkloadType}, {duplicate.K8SWorkloadName}, {duplicate.K8SContainerName}, {duplicate.Image}, {duplicate.ImageId}");
                }
            }
            else
            {
            ...
            foreach (var result in runtimeResults){
                ...
                if (counter % 100 == 0)
                {
                    Console.WriteLine("Processed this many entries: " + counter);
                }

                // Console.WriteLine("Runtime data: " + result.K8SClusterName + "," + result.K8SNamespaceName + "," + result.K8SWorkloadType + "," + result.K8SWorkloadName + "," + result.K8SContainerName + "," + result.Image);
    ...
    static (string secureUrlAuthority, string apiToken, string outputFileName, string reportID) ParseArgs(string[] args)
    {
        if (args.Length < 3)
        {
